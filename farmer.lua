...
food_to_eat                           =
"Nasi Goreng <hq>"                                                --what food to eat (false if none) --there is no check if you don't have that food, script will move on after set time
medicine_to_use                       =
...
do_gc_delivery                        = false --If true, will do GC deliveries using deliveroo everytime retainers are processed
...
verbose                               = true
interval_rate                         = 0.1075 --general wait time used in the script can play with it if your system loads ui more slowly
...
min_orange_scrip_for_exchange   = 1000
min_purple_scrip_for_exchange   = 20
exchange_item_table      = {
    { "orange", 4, 8, 6, 1000 }, --what to spend orange (rroneek tokens here) --only 1 item to spend orange scrips on and one time to purple scrips on
    { "purple", 4, 1, 0, 20 },  --what to spend purple on (high cordials)
}
...
function getAmountOfOrangeScrips()
    local orange_scrips = GetItemCount(41785)
    yield("/echo O: "..orange_scrips)
    if (orange_scrips > min_orange_scrip_for_exchange) then
        if (orange_scrips > 0) then
            return orange_scrips
        end
...
function getAmountOfPurpleScrips()
    local purple_scrips = GetItemCount(33914)
    yield("/echo P: "..purple_scrips)
    if (purple_scrips > min_purple_scrip_for_exchange) then

        if (purple_scrips > 0) then
            return purple_scrips
        end
    end
...
function ScripExchange()
    currentScrips = 0
    while ((getAmountOfOrangeScrips() > 0 or getAmountOfPurpleScrips() > 0)) do
        for i, reward in ipairs(exchange_item_table) do
            doExchange = true
            while (doExchange) do
                if IsAddonVisible("ShopExchangeItemDialog") then
                    yield("ShopExchangeItemDialog")
                    yield("/wait 1")
                    yield("/callback ShopExchangeItemDialog true 0")
                    yield("/wait 1")
                elseif IsAddonVisible("SelectIconString") then
                    yield("SelectIconString")
                    yield("/callback SelectIconString true 0")
                    yield("/wait 1")
                    doExchange = false
                elseif IsAddonVisible("InclusionShop") then
                    yield("/callback InclusionShop true 12 "..reward[2])
                    yield("/wait 1")
                    yield("/callback InclusionShop true 13 "..reward[3])
                    yield("/wait 1")
                    if (reward[1] == "orange") then
                        currentScrips = getAmountOfOrangeScrips()
                    end
                    if (reward[1] == "purple") then
                        currentScrips = getAmountOfPurpleScrips()
                    end
                    yield("/echo Type "..reward[1].." Price "..reward[5].." Amount "..currentScrips)
                    if (currentScrips > 0) then
                        toBuy = (currentScrips//reward[5])
                        if (toBuy > 99) then
                            toBuy = 99
                        end
                        yield("/echo ToBuy "..toBuy)
                        yield("/callback InclusionShop true 14 "..reward[4].." "..toBuy)
                    else
                        yield("/echo Next, not enough scrips")
                        doExchange = false
                    end
