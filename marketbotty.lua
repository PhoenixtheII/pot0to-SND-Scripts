...
function OpenRetainer(r)
    r = r - 1
    if not IsAddonVisible("RetainerList") then
        SomethingBroke("RetainerList", "OpenRetainer(" .. r .. ")")
    end
    yield("/wait 0.3")
    ...
    if not IsAddonVisible("SelectString") then
        SomethingBroke("SelectString", "OpenRetainer(" .. r .. ")")
    end
    yield("/wait 0.3")
    --yield("/click SelectString Entries[3].Select")
    SafeCallback("SelectString", true, 3)
    if not IsAddonVisible("RetainerSellList") then
        SomethingBroke("RetainerSellList", "OpenRetainer(" .. r .. ")")
    end
end
...
function CountItems()
    while IsAddonReady("RetainerSellList") == false do
        yield("/wait 0.1")
    end
    ...
    if (is_hq and hqsearched == false) then
        ...
        while ready == false do
            ...
            if (string.find(search_hits, "hit") and first_price ~= "") and (old_first_price ~= first_price or search_wait_tick > 20) then
                ...
                --[[search_wait_tick = search_wait_tick + 1
                if (search_wait_tick > 500) or (string.find(GetNodeText("ItemSearchResult", 26), "Please wait") and search_wait_tick > 100) then
                    SafeCallback("RetainerSell", true, 4)
                    yield("/wait 0.1")
                    if IsAddonVisible("ItemHistory")==false then
                        SafeCallback("ItemSearchResult", true, 0)
...
function HistoryAverage()
    ...
    ticky = 0
    while first_history == "" and ticky < 10 do
        yield("/wait 0.1")
        first_history = string.gsub(GetNodeText("ItemHistory", 3, 2, 6), "%d", "")
        ticky = ticky + 1
        yield("/echo ticky: " .. ticky)
    end
    yield("/wait 0.1")
    for i = 2, 21 do
...
function CloseSearch()
    while IsAddonVisible("ItemSearchResult") or IsAddonVisible("ItemHistory") do
        ...
        if IsAddonVisible("ItemHistory") then
...
function SafeCallback(...)
    -- Could be safer, but this is a good start, right?
...
if is_read_from_files then
    ...
    if file_exists(file_overrides) and is_using_overrides then
        ...
        chunk()
        or_count = 0
        for _, i in pairs(item_overrides) do
            or_count = or_count + 1
        end
